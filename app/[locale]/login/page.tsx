import { getTranslations } from "next-intl/server";
import { Link } from "@/i18n/routing";
import { auth } from "@/auth";
import { buildPageMetadata } from "@/lib/build-page-metadata";
import { buttonClassName } from "@/components/ui/button";
import { signInWithGoogle, signOutToLocale } from "@/lib/auth/actions";

export async function generateMetadata({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "metadata.login" });
  return buildPageMetadata({
    locale,
    pathname: "/login",
    title: t("title"),
    description: t("description"),
  });
}

export default async function LoginPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  const t = await getTranslations("loginPage");
  const session = await auth();

  const hasDatabase = Boolean(process.env.DATABASE_URL);
  const hasGoogle =
    Boolean(process.env.AUTH_GOOGLE_ID) &&
    Boolean(process.env.AUTH_GOOGLE_SECRET);
  const hasSecret = Boolean(process.env.AUTH_SECRET);
  const authReady = hasDatabase && hasGoogle && hasSecret;

  const email = session?.user?.email ?? session?.user?.name ?? "—";

  return (
    <section className="border-b border-border bg-card py-16 md:py-24">
      <div className="mx-auto max-w-lg px-[max(1rem,env(safe-area-inset-left))] sm:px-6 lg:pr-[max(1rem,env(safe-area-inset-right))]">
        <div className="flex justify-center">
          <span
            className="flex h-16 w-16 items-center justify-center rounded-full border border-border bg-background text-accent"
            aria-hidden
          >
            <UserGlyph className="h-8 w-8" />
          </span>
        </div>
        <h1 className="mt-8 text-center text-2xl font-semibold tracking-tight text-foreground md:text-3xl">
          {t("title")}
        </h1>

        {session?.user ? (
          <>
            <p className="mt-4 text-center text-sm font-medium text-foreground">
              {t("signedInTitle")}
            </p>
            <p className="mt-2 text-center text-sm leading-relaxed text-muted">
              {t("signedInAs", { email })}
            </p>
            <div className="mt-10 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
              <Link href="/app" className={buttonClassName("primary", "min-h-11 px-6")}>
                {t("goToApp")}
              </Link>
              <form action={signOutToLocale.bind(null, locale)}>
                <button
                  type="submit"
                  className={buttonClassName("secondary", "min-h-11 px-6")}
                >
                  {t("signOut")}
                </button>
              </form>
              <Link href="/" className={buttonClassName("ghost", "min-h-11 px-6")}>
                {t("back")}
              </Link>
            </div>
          </>
        ) : (
          <>
            <p className="mt-4 text-center text-sm leading-relaxed text-muted">
              {t("lead")}
            </p>
            <p className="mt-3 text-center text-xs leading-relaxed text-muted">
              {t("note")}
            </p>
            {authReady ? (
              <div className="mt-10 flex justify-center">
                <form action={signInWithGoogle.bind(null, locale)}>
                  <button
                    type="submit"
                    className={buttonClassName(
                      "secondary",
                      "min-h-11 gap-3 px-6 shadow-sm",
                    )}
                  >
                    <GoogleGlyph className="h-5 w-5 shrink-0" />
                    {t("googleCta")}
                  </button>
                </form>
              </div>
            ) : (
              <div className="mt-10 rounded-xl border border-border bg-background/80 p-5 text-left text-sm text-muted">
                <p className="font-semibold text-foreground">{t("setupTitle")}</p>
                <ul className="mt-3 list-disc space-y-2 pl-5">
                  {!hasDatabase ? <li>{t("setupDb")}</li> : null}
                  {!hasGoogle ? <li>{t("setupGoogle")}</li> : null}
                  {!hasSecret ? <li>{t("setupSecret")}</li> : null}
                </ul>
              </div>
            )}
            <div className="mt-10 flex justify-center">
              <Link href="/" className={buttonClassName("ghost", "min-h-11 px-6")}>
                {t("back")}
              </Link>
            </div>
          </>
        )}
      </div>
    </section>
  );
}

function UserGlyph({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="1.5"
      aria-hidden
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z"
      />
    </svg>
  );
}

function GoogleGlyph({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" aria-hidden>
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}
