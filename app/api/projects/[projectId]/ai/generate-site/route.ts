...
import { aiGenerateFullSiteForUser } from "@/lib/projects/ai-workflow";
...
export async function POST(
  ...
  const result = await aiGenerateFullSiteForUser({
  ...
  if (!result.ok) {
    const status = result.error === "forbidden" ? 404 : result.error === "state" ? 409 : 400;
